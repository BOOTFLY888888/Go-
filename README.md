...
这个项目重点是学习后端的知识，以及对前后端流程有一个大概的认识。
...
###实现接口 获取局域网IP GET /api/v1/addresses

思路：
1. 获取电脑在各个局域网的IP地址
2. 转为JSON写入HTTP响应

###实现接口 文件下载 GET /uploads/:path

思路：
1. 将网络路径：path变成本地绝对路径
2. 读取本地文件，写到HTTP响应里

###实现接口 创建二维码 GET /api/v1/qrcodes
...
###实现接口 上传文件 POST /api/files

跟POST /api/v1/texts 类似
区别在于保存文件而不是保存文本
思路：
1. 获取go执行文件所在目录
2. 在该目录创建uploads目录
3. 将文件保存为另一个文件 （看起来多此一举）
4. 返回后者的下载路径
